...
std::string shader_loader::create_autogenerated_stub(shader::technique& technique, shader& asset)
{
    ...
    if (!is_compute)
    {
        ...
        // Create an packed vertex format.
        result += string_format("struct packed_vertex {\n");
        for (ri_data_layout::field& field : layout.layout.fields)
        {
            std::string data_type = ri_data_type_compressed_hlsl_type[static_cast<int>(field.data_type)];
            std::string field_name = field.name;
        ...
        // Create an expanded vertex format.
        ...
        result += " vertex unpacked;\n";
        result += " packed_vertex packed = vertex_buffer.Load<packed_vertex>((vertex_buffer_offset + vertex_id) * sizeof(packed_vertex));\n";

        for (ri_data_layout::field& field : layout.layout.fields)
        {
            if (field.data_type == ri_data_type::t_compressed_unit_vector)
            {
                result += " unpacked." + field.name + " = decompress_unit_vector(packed." + field.name + ");\n";
            }
            else
            {
                result += " unpacked." + field.name + " = packed." + field.name + ";\n";
            }
        }

        result += " return unpacked;\n";
        result += "};\n";
        result += "\n";
