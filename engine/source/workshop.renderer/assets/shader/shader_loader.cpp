...
std::string shader_loader::create_autogenerated_stub(shader::technique& technique, shader& asset)
{
    ...
    // Vulkan has no root-CBV equivalent, so global/draw scope param blocks (the ones DX12
    // binds as a plain cbuffer below) are instead read through a buffer device address passed
    // in via push constants - one address per such block used by this technique, packed as
    // individual uint64_t members (not an array, to sidestep HLSL's array-element padding
    // rules) in the same order as param_block_indices, matching the offsets
    // vulkan_ri_pipeline::create_resources assigns via get_push_constant_offset.
    size_t vulkan_push_constant_count = 0;
    for (uint64_t index : technique.param_block_indices)
    {
        ...
        {
            vulkan_push_constant_count++;
        }
    }

    if (vulkan_push_constant_count > 0)
    {
        result += "#ifdef TARGET_VULKAN\n";
        result += "struct vk_param_block_addresses_t {\n";
        for (size_t i = 0; i < vulkan_push_constant_count; i++)
        {
            result += string_format("\tuint64_t addr_%zi;\n", i);
        }
        result += "};\n";
        result += "[[vk::push_constant]] vk_param_block_addresses_t vk_param_block_addresses;\n";
        result += "#endif\n";
        result += "\n";
    }

    // Add param block struct definitions.
    size_t vulkan_push_constant_index = 0;
    for (uint64_t index : technique.param_block_indices)
    {
        shader::param_block& block = asset.param_blocks[index];

        bool is_bindless_scope = (block.scope == ri_data_scope::instance || block.scope == ri_data_scope::indirect);

        // Build the field list once - it's reused as both the vulkan struct-load target and
        // the dx12 cbuffer body below, the two only differ in how the fields end up bound.
        //
        // Note: the per-field vulkan accessors below are real "static const" global variable
        // declarations, not #define macros - a macro on a bare field name (eg. "shadow_map_count")
        // would blindly rewrite *any* occurrence of that token in the file, including unrelated
        // member accesses like "light.shadow_map_count" on some other struct entirely. Real
        // declarations go through normal HLSL name resolution instead, which only matches bare
        // identifier references, exactly like the cbuffer members they're standing in for.
        std::string field_lines;
        std::string vulkan_field_globals;
        ...
        for (ri_data_layout::field& field : block.layout.fields)
        {
            ...
            field_lines += string_format("\t%s %s;\n", data_type.c_str(), field_name.c_str());

            if (!is_bindless_scope)
            {
                vulkan_field_globals += string_format(
                    "static const %s %s = %s_vk_loaded.%s;\n",
                    data_type.c_str(), field_name.c_str(), block.name.c_str(), field_name.c_str());
            }
        }
        ...
        {
            result += string_format("struct %s_data {\n", block.name.c_str());
            result += field_lines;
            result += "};\n";
            result += "#ifdef TARGET_VULKAN\n";
            result += string_format(
                "static const %s_data %s_vk_loaded = vk::RawBufferLoad<%s_data>(vk_param_block_addresses.addr_%zi);\n",
                block.name.c_str(), block.name.c_str(), block.name.c_str(), vulkan_push_constant_index);
            result += vulkan_field_globals;
            result += "#else\n";
...
bool shader_loader::preprocess_replaced_function(const char* path, const char*& read_ptr, std::string& result, std::string& stub, int function_index)
{
    lexer lex;
    token tok;
    lex.load(read_ptr);
    ...
    if (lex.expect(tok, token_type::literal_identifier))
    {
        function_name = tok.buffer;
    } 
    else
    {
        db_error(asset, "[%s] Could not preprocess function '%s'.", path, tok.buffer);
        return false;
    }

    // Read start of argument list.
    if (!lex.expect(tok, token_type::op_parenthesis_open))
    {
    ...
    if (!lex.expect(tok, token_type::op_semicolon))
    {
        db_error(asset, "[%s] Malformed preprocessor function '%s', expected ).", path, function_name.c_str());
        return false;
    }

    // Generate code in debug builds.
    std::string format = "";
    int category = 0;
    std::vector<int> args_to_store;
    std::vector<int> args_to_pass;

    // log messages
    if (function_name == "db_verbose" ||
        function_name == "db_log" ||
        function_name == "db_success" ||
        function_name == "db_warning" ||
    ...
    int type = (int)render_gpu_log_type::log;
    if (function_name == "db_verbose")
    {
        type = (int)render_gpu_log_type::verbose;
    }
    else if (function_name == "db_log")
    {
        type = (int)render_gpu_log_type::log;
    }
    else if (function_name == "db_success")
    {
        type = (int)render_gpu_log_type::success;
    }
    else if (function_name == "db_warning")
    {
        type = (int)render_gpu_log_type::warning;
    }
    else if (function_name == "db_error")
    {
        type = (int)render_gpu_log_type::error;
    }
    else if (function_name == "db_fatal")
    {
        type = (int)render_gpu_log_type::fatal;
    }
    else if (function_name == "db_assert" || function_name == "db_assert_message")
    {
        type = (int)render_gpu_log_type::assert;
    }

    // Build generated function
    {
        stub += string_format("void %s(", generated_func_name.c_str());
        for (size_t i = 0; i < args_to_pass.size(); i++)
        {
            ...
            stub += string_format("uint arg%i", i);
        }
        stub += ")\n";
        stub += "{\n";

            // If its an assert check the first argument is failing before emitting anything.
            if (function_name == "db_assert" || 
                function_name == "db_assert_message")
            {
                stub += "\tif (arg0) return;\n";
            }

            // If you modify any of this, make sure you update render_system_gpu_logs so it can be read.

            // Calculate bytes needed to add.
            stub += "\tuint write_ptr;\n";
            stub += string_format("\tuint bytes_required = %i;\n", bytes_required);
            ...
            for (size_t i = 0; i < format.size(); i += 4)
            {
                ...
                if (i < format.size())
                {
                    dword |= ((uint32_t)format[i]) << 0;
                    text += format[i];
                }
                if (i + 1 < format.size())
                {
                    dword |= ((uint32_t)format[i + 1]) << 8;
                    text += format[i + 1];
                }
                if (i + 2 < format.size())
                {
                    dword |= ((uint32_t)format[i + 2]) << 16;
                    text += format[i + 2];
                }
                if (i + 3 < format.size())
                {
                    dword |= ((uint32_t)format[i + 3]) << 24;
                    text += format[i + 3];
                }
            ...
            for (size_t i = 0; i < args_to_store.size(); i++)
            {
                size_t arg_index = 0;
                for (size_t j = 0; j < args_to_pass.size(); j++)
                {
                    if (args_to_store[i] == args_to_pass[j])
                    {
                        arg_index = j;
                        break;
                    }
                ...
                stub += string_format("\t\tdebug_output_buffer.Store(write_ptr + %i, arg%i);\n", offset, arg_index);     // argument DWORD
                offset += 4;
            }
    ...
    // Call the generated function in code
    result += string_format("%s(", generated_func_name.c_str());
    for (size_t i = 0; i < args_to_pass.size(); i++)
    {
        // bitcast to dword
    ...
    read_ptr = lex.get_cursor();

    return true;
}

bool shader_loader::preprocess_source(const char* path, std::string& source, std::string& stub)
{
    const char* replaced_function_names[] = {
        "db_verbose",
        "db_log",
        "db_success",
        "db_warning",
    ...
    const char* read_ptr = source.c_str();
    while (read_ptr[0] != '\0')
    {
        bool is_replaced_function = false;

        for (size_t j = 0; j < sizeof(replaced_function_names) / sizeof(*replaced_function_names); j++)
        {
            const char* func_name = replaced_function_names[j];
            size_t func_name_len = strlen(func_name);
            if (strncmp(read_ptr, func_name, func_name_len) == 0 && read_ptr[func_name_len] == '(')
            {
                is_replaced_function = true;
                break;
            }
        }
        ...
        {
            if (!preprocess_replaced_function(path, read_ptr, result, stub, ++function_index))
            {
                return false;
            }
        }
...
bool shader_loader::compile_shader_stage(const char* path, shader::technique& technique, shader& asset, platform_type asset_platform, config_type asset_config, shader::shader_stage& stage, ri_shader_stage pipeline_stage)
{
    ...
    // Preprocess text to replace things like db_log
    std::string preprocess_stub = "";
    if (!preprocess_source(path, source_code, preprocess_stub))
    {
        return false;
    }
...
bool shader_loader::compile(const char* input_path, const char* output_path, platform_type asset_platform, config_type asset_config, asset_flags flags)
{
    ...
    if (!add_implicit_param_block("debug_info"))
    {
        return false;
    }
