...
std::string shader_loader::create_autogenerated_stub(shader::technique& technique, shader& asset)
{
    ...
    for (uint64_t index : technique.param_block_indices)
    {
        ...
        for (ri_data_layout::field& field : block.layout.fields)
        {
            ...
            result += string_format("#define %s %s[NonUniformResourceIndex(%s_index)]\n",
                field.name.c_str(),
                table_name.c_str(), 
                field.name.c_str()
            );
    ...
    result += "#define load_vertex(vertex_id) table_byte_buffers[NonUniformResourceIndex(vertex_buffer_index)].Load<vertex>((vertex_buffer_offset + vertex_id) * sizeof(vertex))\n";
    result += "\n";
...
bool shader_loader::compile(const char* input_path, const char* output_path, platform_type asset_platform, config_type asset_config)
{
    ...
    vertex_info_block.scope = ri_data_scope::instance;
