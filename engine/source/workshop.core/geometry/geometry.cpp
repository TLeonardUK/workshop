...
size_t geometry::add_mesh(const char* name, size_t material_index, const std::vector<uint32_t>& indices, const aabb& bounds, float min_texel_area, float max_texel_area, float avg_texel_area, float min_world_area, float max_world_area, float avg_world_area, float uv_density)
{
...
std::unique_ptr<geometry> geometry::load(const char* path, const geometry_load_settings& settings)
{
    ...
    {
        result = geometry_assimp_loader::load(buffer, path, settings);
    }
    else
    {
