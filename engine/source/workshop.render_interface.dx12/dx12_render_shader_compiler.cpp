...
public:
    ...
    HRESULT STDMETHODCALLTYPE LoadSource(_In_ LPCWSTR pFilename, _COM_Outptr_result_maybenull_ IDxcBlob** ppIncludeSource) override
    {
        ...
        HRESULT ret = m_library->CreateBlobWithEncodingOnHeapCopy(source.c_str(), strlen(source.c_str()), CP_UTF8, sourceBlob.GetAddressOf());
        if (FAILED(ret))
        {
...
dx12_render_shader_compiler::dx12_render_shader_compiler(dx12_render_interface& renderer)
    : m_renderer(renderer)
{
}

dx12_render_shader_compiler::~dx12_render_shader_compiler()
{
    m_library = nullptr;
    m_compiler = nullptr;
}

result<void> dx12_render_shader_compiler::create_resources()
{
...
void dx12_render_shader_compiler::parse_output(render_shader_compiler_output& output, const std::string& text)
{
    ...
    render_shader_compiler_output::log log;
    std::string log_type = "";
    ...
    for (size_t i = 0; i < lines.size(); i++)
    {
        ...
        if (std::regex_match(line, match, error_pattern))
        {
            ...
            log.line = from_string<size_t>(match[2].str());
            log.column = from_string<size_t>(match[3].str());
...
render_shader_compiler_output dx12_render_shader_compiler::compile(
    ...
    bool debug)
{
    std::array<std::wstring, static_cast<int>(render_shader_stage::COUNT)> stage_target_profiles = {
        L"vs_6_2",
        L"ps_6_2",
        L"ds_6_2",
        L"hs_6_2",
    ...
    render_shader_compiler_output output;
    ...
    Microsoft::WRL::ComPtr<IDxcBlobEncoding> sourceBlob;
    HRESULT ret = m_library->CreateBlobWithEncodingOnHeapCopy(source, strlen(source), CP_UTF8, sourceBlob.GetAddressOf());
    if (FAILED(ret))
    {
